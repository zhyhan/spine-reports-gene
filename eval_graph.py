...
tf.app.flags.DEFINE_string(
    'eval_dir', 'tmp/graph/tfmodels_%s/'%FLAGS.Fold, 'Directory where the results are saved to.')

tf.app.flags.DEFINE_string('checkpoint_dir', 'tmp/graph/tfmodels_%s/'%FLAGS.Fold,
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        feature_embedding, logits= SpinePathNet.g_net_graph(image, Fold=1, batch_size=FLAGS.batch_size,\
