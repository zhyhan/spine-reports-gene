...
tf.app.flags.DEFINE_string(
    'eval_dir', 'tmp/tfmodels_%s/'%FLAGS.Fold, 'Directory where the results are saved to.')

tf.app.flags.DEFINE_string('checkpoint_dir', 'tmp/tfmodels_%s/'%FLAGS.Fold,
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        feature_embedding, logits= SpinePathNet.net(image, batch_size=FLAGS.batch_size,\
                                       class_num = FLAGS.num_classes, reuse=False,\
                                       is_training=False, scope='SpinePathNet')  
